...
class main extends React.Component {
	...
	render() {
		...
		const isRequired = viewattr === 3 || viewattr=== '3';
		...
		const req = classNames({
            'required': viewattr === 3 || viewattr === '3',
        });
		console.log(req);
		...
		if(viewattr === 1) {
			...
			if(isHtmlArea) {
				eles = <textarea className="required" id={`field${fieldid}`} name={`field${fieldid}`} rows={textheight}  style={styleInfo}/>;
			} else {
