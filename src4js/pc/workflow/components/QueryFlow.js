...
class QueryFlow extends React.Component {
    ...
    render() {
        ...
        const formShow = <Form horizontal>{this.getSearchs()}</Form>
        return (
            <div className='wea-workflow-query'>
                ...
                <WeaNewTop showDropIcon={true} title={title} loading={loading} icon={<i className='icon-portal-workflow'/>} iconBgcolor='#55D2D4' buttonSpace={10} buttons={this.getButtons()} hideButtons={this.getRightMenu()}>
                    <div style={{height: '100%',display:(showTable ? 'block' : 'none')}}>
                    	<WeaLayoutR11 leftCom={this.getTree()} leftWidth={25} >
	                        <WeaTab
	                        	...
	                        	onSearchChange={v=>{actions.saveFields({...fields.toJS(),requestname:{name:'requestname',value:v}})}}
	                            />
	                        <WeaNewTableOld
	                            current={current}
	                        	pageSize={pageSize}
                    ...
                    <div className='wea-workflow-query-search' style={{display:(!showTable ? 'block' : 'none')}}>
                        {formShow}
    ...
    getSearchs() {
        ...
        return [
            (<WeaSearchGroup needTigger={showTable} title={this.getTitle()} showGroup={this.isShowFields()} items={this.getFields()}/>),
            (<WeaSearchGroup needTigger={showTable} title={this.getTitle(1)} showGroup={this.isShowFields(1)} items={this.getFields(1)}/>)
        ]
    }
    getTitle(index = 0) {
	...
	getRightMenu(){
    	const {searchParamsAd,actions,showTable} = this.props;
    	let btns = [];
    	btns.push(<a onClick={()=>{actions.doSearch();actions.updateDisplayTable(true);;actions.setShowSearchAd(false)}}><i className='' style={{marginRight:10,verticalAlign:'middle'}} />搜索</a>)
    	if(showTable){
        	btns.push(<a onClick={()=>{selectedRowKeys && `${selectedRowKeys.toJS()}` ? actions.batchShareWf(`${selectedRowKeys.toJS()}`) :Modal.warning({
                title: '请至少选择一项'})}}  ><i className='' style={{marginRight:10,verticalAlign:'middle'}} />批量共享</a>);
...
QueryFlow = createForm({
    onFieldsChange(props, fields) {
        props.actions.saveFields(objectAssign({}, props.fields.toJS(), fields));
    },
