...
class Req extends React.Component {
    ...
    render() {
        const {reqLoadDuration,jsLoadDuration,apiDuration,dispatchDuration,
            ...
            resourcesOperates,resourcesCount,resourcesColumns,resourcesCurrent,resourcesTabKey,reqTabKey,logListTabKey,isShowSignInput,initSignInput,scriptcontent,
    ...
    getButtons() {
        ...
        rightMenu && !is(rightMenu,Immutable.fromJS({})) && rightMenu.get('rightMenus').map(m=>{
        	...
        	//console.log(btnArr.length,m.get('isTop') == '1' && btnArr.length < 4,m.get('menuName'),m.get('menuIcon'),m.get('menuFun'));
...
//      btnArr.push(<Button type="ghost" onClick={()=>{router.push(`/main/workflow/${pathBack}`)}}>返回</Button>)
    ...
    getHideButtons() {
        ...
        rightMenu && !is(rightMenu,Immutable.fromJS({})) && rightMenu.get('rightMenus').map(m=>{
        	let fn = m.get('menuFun').indexOf('this') >= 0 ? `${m.get('menuFun').split('this')[0]})` : m.get('menuFun');
	        btnArr.push(<a href="javascript:void(0)" onClick={()=>{eval(fn)}}><i className={m.get('menuIcon')} style={{marginRight:10,verticalAlign:'middle'}} />{m.get('menuName')}</a>)
        });
        return btnArr
    }
