...
import * as ContactsAction from '../../../actions/homepage/contacts';
//通讯录元素
class Contacts extends Component {
	handleChangeTab(tabid) {
		...
		actions.getContactsData('', data.url, tabid);
	}
	render() {
		const {
			eid,
		...
		{_isEmptyObject(cdata.data) ? null: <ContactsContent list={ cdata.data} esetting={data.esetting}/>}
		</div>
	}
}
...
const ContactsTitle = ({
	...
	return <div id={`titleContainer_${eid}`} className="titlecontainer"> 
        ...
        {!_isEmptyObject(toolbar) ? <div className="optoolbar">
...
const ContactsContent = ({
	list,
	esetting
}) => list.length !== 0 ? <div style={{overflowY:'scroll',maxHeight:'205px'}}><Table columns={formatData(list[0], esetting)} showHeader={false} pagination={false} dataSource={list} size="small"/></div> : <div></div>
