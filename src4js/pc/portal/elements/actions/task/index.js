...
import { reqTaskDatas } from '../../apis/task';
import Immutable from 'immutable';
import ecLocalStorage from '../../util/ecLocalStorage';

import { initTaskTabDatas } from './tab';
import { initTitleData } from './title';
const initTaskConfig = (config) => {
    return (dispatch, getState) => {
        ...
        dispatch(getTaskDatas(params));
    }
}
const getTaskDatas = (params) => {
    return (dispatch, getState) => {
        const { eid } = params;
        reqTaskDatas(params).then((data) => {
            let iolddata = getState().elements.get("data");
            let idata = Immutable.fromJS(data);
            ecLocalStorage.set("portal-" + window.global_hpid, "task-" + eid, data, true);
            dispatch(handleImmutableData(eid, false, data));
            dispatch(initTitleData(eid,data.currenttab));
            dispatch(initTaskTabDatas(eid, data.currenttab, data.data));
        });
    }
}
