...
import { reqNewNoticeDatas } from '../../apis/newnotice';
import Immutable from 'immutable';
import ecLocalStorage from '../../util/ecLocalStorage';
const initNewNoticeConfig = (config) => {
    return (dispatch, getState) => {
        ...
        dispatch(getNewNoticeDatas(params));
    }
}
const getNewNoticeDatas = (params) => {
    return (dispatch, getState) => {
        const { eid } = params;
        reqNewNoticeDatas(params).then((data) => {
            let iolddata = getState().elements.get("data");
            let idata = Immutable.fromJS(data);
            ecLocalStorage.set("portal-" + window.global_hpid, "newnotice-" + eid, data, true);
