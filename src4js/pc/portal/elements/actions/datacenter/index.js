...
import { reqDataCenterDatas } from '../../apis/datacenter';
import Immutable from 'immutable';
import ecLocalStorage from '../../util/ecLocalStorage';
const initDataCenterConfig = (config) => {
    return (dispatch, getState) => {
        ...
        dispatch(getDataCenterDatas(params));
    }
}
const getDataCenterDatas = (params) => {
    return (dispatch, getState) => {
        const { eid } = params;
        reqDataCenterDatas(params).then((data) => {
            let iolddata = getState().elements.get("data");
            let idata = Immutable.fromJS(data);
            ecLocalStorage.set("portal-" + window.global_hpid, "datacenter-" + eid, data, true);
