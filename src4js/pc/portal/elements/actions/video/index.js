...
import { reqVideoDatas } from '../../apis/video';
import Immutable from 'immutable';
import ecLocalStorage from '../../util/ecLocalStorage';
const initVideoConfig = (config) => {
    return (dispatch, getState) => {
        ...
        dispatch(getVideoDatas(params));
    }
}
const getVideoDatas = (params) => {
    return (dispatch, getState) => {
        const { eid } = params;
        reqVideoDatas(params).then((data) => {
            let iolddata = getState().elements.get("data");
            let idata = Immutable.fromJS(data);
            ecLocalStorage.set("portal-" + window.global_hpid, "video-" + eid, data, true);
