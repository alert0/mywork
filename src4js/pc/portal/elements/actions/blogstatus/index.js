...
import { reqBlogStatusDatas } from '../../apis/blogstatus';
import Immutable from 'immutable';
import ecLocalStorage from '../../util/ecLocalStorage';

import { initBlogStatusTabDatas } from './tab';
import { initTitleData } from './title';
const initBlogStatusConfig = (config) => {
    return (dispatch, getState) => {
        ...
        dispatch(getBlogStatusDatas(params));
    }
}
const getBlogStatusDatas = (params) => {
    return (dispatch, getState) => {
        const { eid } = params;
        reqBlogStatusDatas(params).then((data) => {
            let iolddata = getState().elements.get("data");
            let idata = Immutable.fromJS(data);
            ecLocalStorage.set("portal-" + window.global_hpid, "blogstatus-" + eid, data, true);
