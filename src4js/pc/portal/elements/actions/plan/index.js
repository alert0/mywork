...
import { reqPlanDatas } from '../../apis/plan';
import Immutable from 'immutable';
import ecLocalStorage from '../../util/ecLocalStorage';
const initPlanConfig = (config) => {
    return (dispatch, getState) => {
        ...
        dispatch(getPlanDatas(params));
    }
}
const getPlanDatas = (params) => {
    return (dispatch, getState) => {
        const { eid } = params;
        reqPlanDatas(params).then((data) => {
            let iolddata = getState().elements.get("data");
            let idata = Immutable.fromJS(data);
            ecLocalStorage.set("portal-" + window.global_hpid, "plan-" + eid, data, true);
