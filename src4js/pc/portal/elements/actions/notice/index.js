...
import { reqNoticeDatas } from '../../apis/notice';
import Immutable from 'immutable';
import ecLocalStorage from '../../util/ecLocalStorage';
const initNoticeConfig = (config) => {
    return (dispatch, getState) => {
        ...
        dispatch(getNoticeDatas(params));
    }
}
const getNoticeDatas = (params) => {
    return (dispatch, getState) => {
        const { eid } = params;
        reqNoticeDatas(params).then((data) => {
            let iolddata = getState().elements.get("data");
            let idata = Immutable.fromJS(data);
            ecLocalStorage.set("portal-" + window.global_hpid, "notice-" + eid, data, true);
