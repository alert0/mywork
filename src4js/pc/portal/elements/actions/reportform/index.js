...
import { reqReportFormDatas } from '../../apis/reportform';
import Immutable from 'immutable';
import ecLocalStorage from '../../util/ecLocalStorage';

import { initReportFormTabDatas } from './tab';
import { initTitleData } from './title';
const initReportFormConfig = (config) => {
    return (dispatch, getState) => {
        ...
        dispatch(getReportFormDatas(params));
    }
}
const getReportFormDatas = (params) => {
    return (dispatch, getState) => {
        const { eid } = params;
        reqReportFormDatas(params).then((data) => {
            let iolddata = getState().elements.get("data");
            let idata = Immutable.fromJS(data);
            ecLocalStorage.set("portal-" + window.global_hpid, "reportform-" + eid, data, true);
            dispatch(handleImmutableData(eid, false, data));
            dispatch(initTitleData(eid,data.currenttab));
            dispatch(initReportFormTabDatas(eid, data.currenttab, data.data));
        });
    }
}
