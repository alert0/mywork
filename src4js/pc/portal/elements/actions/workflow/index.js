...
import { reqWorkflowDatas } from '../../apis/workflow';
import Immutable from 'immutable';
import ecLocalStorage from '../../util/ecLocalStorage';

import { initWorkflowTabDatas } from './tab';
import { initTitleData } from './title';
const initWorkflowConfig = (config) => {
    return (dispatch, getState) => {
        ...
        dispatch(getWorkflowDatas(params));
    }
}
const getWorkflowDatas = (params) => {
    return (dispatch, getState) => {
        const { eid } = params;
        reqWorkflowDatas(params).then((data) => {
            let iolddata = getState().elements.get("data");
            let idata = Immutable.fromJS(data);
            ecLocalStorage.set("portal-" + window.global_hpid, "workflow-" + eid, data, true);
            dispatch(handleImmutableData(eid, false, data));
            dispatch(initTitleData(eid,data.currenttab));
            dispatch(initWorkflowTabDatas(eid, data.currenttab, data.data));
        });
    }
}
