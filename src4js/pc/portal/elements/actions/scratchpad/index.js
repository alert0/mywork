...
import { reqScratchpadDatas } from '../../apis/scratchpad';
import Immutable from 'immutable';
import ecLocalStorage from '../../util/ecLocalStorage';
const initScratchpadConfig = (config) => {
    return (dispatch, getState) => {
        ...
        dispatch(getScratchpadDatas(params));
    }
}
const getScratchpadDatas = (params) => {
    return (dispatch, getState) => {
        const { eid } = params;
        reqScratchpadDatas(params).then((data) => {
            let iolddata = getState().elements.get("data");
            let idata = Immutable.fromJS(data);
            ecLocalStorage.set("portal-" + window.global_hpid, "scratchpad-" + eid, data, true);
