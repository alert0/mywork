...
import { reqFlashDatas } from '../../apis/flash';
import Immutable from 'immutable';
import ecLocalStorage from '../../util/ecLocalStorage';
const initFlashConfig = (config) => {
    return (dispatch, getState) => {
        ...
        dispatch(getFlashDatas(params));
    }
}
const getFlashDatas = (params) => {
    return (dispatch, getState) => {
        const { eid } = params;
        reqFlashDatas(params).then((data) => {
            let iolddata = getState().elements.get("data");
            let idata = Immutable.fromJS(data);
            ecLocalStorage.set("portal-" + window.global_hpid, "flash-" + eid, data, true);
