...
import { reqViewDatas } from '../../apis/view';
import Immutable from 'immutable';
import ecLocalStorage from '../../util/ecLocalStorage';
const initViewConfig = (config) => {
    return (dispatch, getState) => {
        ...
        dispatch(getViewDatas(params));
    }
}
const getViewDatas = (params) => {
    return (dispatch, getState) => {
        const { eid } = params;
        reqViewDatas(params).then((data) => {
            let iolddata = getState().elements.get("data");
            let idata = Immutable.fromJS(data);
            ecLocalStorage.set("portal-" + window.global_hpid, "view-" + eid, data, true);
