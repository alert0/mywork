...
import { reqSlideDatas } from '../../apis/slide';
import Immutable from 'immutable';
import ecLocalStorage from '../../util/ecLocalStorage';
const initSlideConfig = (config) => {
    return (dispatch, getState) => {
        ...
        dispatch(getSlideDatas(params));
    }
}
const getSlideDatas = (params) => {
    return (dispatch, getState) => {
        const { eid } = params;
        reqSlideDatas(params).then((data) => {
            let iolddata = getState().elements.get("data");
            let idata = Immutable.fromJS(data);
            ecLocalStorage.set("portal-" + window.global_hpid, "slide-" + eid, data, true);
