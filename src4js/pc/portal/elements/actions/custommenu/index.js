...
import { reqCustomMenuDatas } from '../../apis/custommenu';
import Immutable from 'immutable';
import ecLocalStorage from '../../util/ecLocalStorage';
const initCustomMenuConfig = (config) => {
    return (dispatch, getState) => {
        ...
        dispatch(getCustomMenuDatas(params));
    }
}
const getCustomMenuDatas = (params) => {
    return (dispatch, getState) => {
        const { eid } = params;
        reqCustomMenuDatas(params).then((data) => {
            let iolddata = getState().elements.get("data");
            let idata = Immutable.fromJS(data);
            ecLocalStorage.set("portal-" + window.global_hpid, "custommenu-" + eid, data, true);
