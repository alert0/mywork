...
class JobsInfo extends React.Component {
    ...
    render() {
        ...
        if(isHasRight === 'true'){
            if (!_isEmpty(edata)) {
                const list = edata.data;
                if(list.length > 0)
                    EContentHtml = <Table columns = {formatData(list[0], edata.esetting)} showHeader={false} pagination={false} dataSource={list} size="small"/>
            }
        }else{
...
JobsInfo = WeaTools.tryCatch(React, MyErrorHandler, { error: "" })(JobsInfo);
...
function mapDispatchToProps(dispatch) {
    return {
        actions: bindActionCreators(JobsInfoAction, dispatch)
    }
}
...
function mergeProps(stateProps, dispatchProps, ownProps) {
    ...
    return {
        ...
        data: stateProps.data.get(eid) || Immutable.fromJS(ecLocalStorage.getObj("portal-" + window.global_hpid, "jobsinfo-" + eid, true) || {}),
        config: Immutable.fromJS(config),
        actions: dispatchProps.actions
    };
}
export default connect(mapStateToProps, mapDispatchToProps, mergeProps)(JobsInfo);
