...
import * as EContentAction from '../../actions/rss/tab';
...
class EContent extends React.Component {
    ...
    render() {
        ...
        if (!_isEmpty(tabdata)) {
            ...
            const tabsetting = tabdata.tabsetting;
            const { isIE, readType, mess } = tabsetting;
            if(readType === '1' && isIE === 'false')
                contentHtml = <div>&nbsp;&nbsp;{mess}</div>
        }
...
const mapStateToProps = state => {
    const { ersstab } = state;
    return ({
        data: ersstab.get("data"),
        tabid: ersstab.get("tabid"),
        refresh: ersstab.get("refresh")
    })
}
...
function mergeProps(stateProps, dispatchProps, ownProps) {
    ...
    return {
        data: stateProps.data.get(key) || Immutable.fromJS(ecLocalStorage.getObj("portal-" + window.global_hpid, "rss-tab-" + key, true) || initdata),
        tabid: tabid,
