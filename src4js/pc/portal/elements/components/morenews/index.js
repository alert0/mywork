...
import * as MoreNewsAction from '../../actions/morenews/';
...
class MoreNews extends React.Component {
    componentDidMount(){
        ...
        if(!_isEmpty(config.toJSON())){
            actions.initMoreNewsConfig(config.toJSON());
        }
    }
    ...
    render() {
        ...
        if(isHasRight === 'true'){
            if (!_isEmpty(edata)) {
                EContentHtml = edata.data.map(item => {
                    return <div style={{marginBottom:'8px'}}>
                     {item.img !== '' ? <div style={{float:'left'}}><img height="142" width="142" src={item.img}/></div>:null}
                    <Table columns={formatData(item.data[0], edata.esetting)} showHeader={false} pagination={false} dataSource={item.data} size="small"/>
                </div>
                }); 
            }
        }else{
...
function mapDispatchToProps(dispatch) {
    return {
        actions: bindActionCreators(MoreNewsAction, dispatch)
    }
}
...
function mergeProps(stateProps, dispatchProps, ownProps) {
    ...
    return {
        ...
        data: stateProps.data.get(eid) || Immutable.fromJS(ecLocalStorage.getObj("portal-" + window.global_hpid, "morenews-" + eid, true) || {}),
        config: Immutable.fromJS(config),
        actions: dispatchProps.actions
    };
}
export default connect(mapStateToProps, mapDispatchToProps, mergeProps)(MoreNews);
