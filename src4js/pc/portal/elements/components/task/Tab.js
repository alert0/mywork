...
import * as EContentAction from '../../actions/task/tab';
...
class EContent extends React.Component {
    ...
    render() {
        ...
        const { tabids, titles, esetting, params, counts } = edata.toJSON();
        const { eid } = params;
        params['tabid'] = tabid;
        let tabdata = data.toJSON();
        if (!_isEmpty(tabdata)) {
            contentHtml =  <TaskCom eid={eid} data={tabdata} esetting={esetting} />
        }
        if(refresh) contentHtml = <Spin>{ contentHtml }</Spin>
        return <div>
            <TitleContainer params={params} counts={counts} config={config} titles={titles} tabids={tabids} handleRefresh={handleRefresh}/>
...
const mapStateToProps = state => {
    const { etasktab } = state;
    return ({
        data: etasktab.get("data"),
        tabid: etasktab.get("tabid"),
        refresh: etasktab.get("refresh")
    })
}
...
function mergeProps(stateProps, dispatchProps, ownProps) {
    ...
    return {
        data: stateProps.data.get(key) || Immutable.fromJS(ecLocalStorage.getObj("portal-" + window.global_hpid, "task-tab-" + key, true) || initdata),
        tabid: tabid,
