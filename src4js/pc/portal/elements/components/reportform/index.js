...
import * as ReportFormAction from '../../actions/reportform/';
...
class ReportForm extends React.Component {
    componentDidMount(){
        ...
        if(!_isEmpty(config.toJSON())){
            actions.initReportFormConfig(config.toJSON());
        }
    }
...
ReportForm = WeaTools.tryCatch(React, MyErrorHandler, { error: "" })(ReportForm);
...
function mapDispatchToProps(dispatch) {
    return {
        actions: bindActionCreators(ReportFormAction, dispatch)
    }
}
...
function mergeProps(stateProps, dispatchProps, ownProps) {
    ...
    return {
        ...
        data: stateProps.data.get(eid) || Immutable.fromJS(ecLocalStorage.getObj("portal-" + window.global_hpid, "reportform-" + eid, true) || {}),
