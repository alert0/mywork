...
class Video extends React.Component {
    componentDidMount(){
        ...
        if(!_isEmpty(config.toJSON())){
            actions.initVideoConfig(config.toJSON());
        }
    }
    ...
    render() {
        ...
        if(isHasRight === 'true'){
            if (!_isEmpty(edata)) {
                EContentHtml = <VideoCom data={edata.data} eid={eid}/>;
            }
        }else{
...
function mapDispatchToProps(dispatch) {
    return {
        actions: bindActionCreators(VideoAction, dispatch)
    }
}
...
function mergeProps(stateProps, dispatchProps, ownProps) {
    ...
    return {
        ...
        data: stateProps.data.get(eid) || Immutable.fromJS(ecLocalStorage.getObj("portal-" + window.global_hpid, "video-" + eid, true) || {}),
