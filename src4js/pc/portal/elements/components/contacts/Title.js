...
import * as ContactsTitleAction from '../../actions/contacts/title';
...
class TitleContainer extends React.Component {
    ...
    render() {
        ...
        const { tabid, params, config, tabids, titles, actions, handleRefresh } = this.props;
        ...
        return <div id={ `titleContainer_${eid}` } className={`titlecontainer ${className}`}>
                ...
                {config.item.header.canHeadbar === 'false' ? <OpToolbar config={config} clsname={'nodisplay'} handleRefresh={handleRefresh}/> : null} 
              </div>
      }
}
...
const mapStateToProps = state => {
    const { econtactstitle } = state;
    return ({
        tabid: econtactstitle.get("tabid")
    })
}

function mapDispatchToProps(dispatch) {
    return {
        actions: bindActionCreators(ContactsTitleAction, dispatch)
    }
}
