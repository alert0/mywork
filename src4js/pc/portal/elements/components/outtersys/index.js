...
import * as OutterSysAction from '../../actions/outtersys/';
import Immutable from 'immutable';
import EHeader from '../common/EHeader';
import { NoRightCom } from '../Common';
import OutterSysCom from './OutterSys';
//元素组件
class OutterSys extends React.Component {
    componentDidMount(){
        const { config, actions } = this.props;
        if(!_isEmpty(config.toJSON())){
            actions.initOutterSysConfig(config.toJSON());
        }
    }
    ...
    render() {
        ...
        if(isHasRight === 'true'){
            if (!_isEmpty(edata)) {
                EContentHtml = <OutterSysCom data={edata.data} eid={eid} esetting={edata.esetting}/>;
            }
        }else{
...
OutterSys = WeaTools.tryCatch(React, MyErrorHandler, { error: "" })(OutterSys);
...
function mapDispatchToProps(dispatch) {
    return {
        actions: bindActionCreators(OutterSysAction, dispatch)
    }
}
...
function mergeProps(stateProps, dispatchProps, ownProps) {
    ...
    return {
        ...
        data: stateProps.data.get(eid) || Immutable.fromJS(ecLocalStorage.getObj("portal-" + window.global_hpid, "outtersys-" + eid, true) || {}),
        config: Immutable.fromJS(config),
        actions: dispatchProps.actions
    };
}
export default connect(mapStateToProps, mapDispatchToProps, mergeProps)(OutterSys);
