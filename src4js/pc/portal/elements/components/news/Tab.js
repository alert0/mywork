...
import * as EContentAction from '../../actions/news/tab';
...
class EContent extends React.Component {
    ...
    render() {
        ...
        if (!_isEmpty(tabdata)) {
            ...
            const { rollDirection } = tabdata.tabsetting
            contentHtml = <MarqueeCom eid={eid} scolltype={rollDirection}><NewsType eid={eid} data={tabdata} esetting={esetting}/></MarqueeCom>
        }
...
const mapStateToProps = state => {
    const { enewstab } = state;
    return ({
        data: enewstab.get("data"),
        tabid: enewstab.get("tabid"),
        refresh: enewstab.get("refresh")
    })
}
...
function mergeProps(stateProps, dispatchProps, ownProps) {
    ...
    return {
        data: stateProps.data.get(key) || Immutable.fromJS(ecLocalStorage.getObj("portal-" + window.global_hpid, "news-tab-" + key, true) || initdata),
        tabid: tabid,
