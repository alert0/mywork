...
BlogStatus = WeaTools.tryCatch(React, MyErrorHandler, { error: "" })(BlogStatus);
...
function mapDispatchToProps(dispatch) {
    return {
        actions: bindActionCreators(BlogStatusAction, dispatch)
    }
}
...
function mergeProps(stateProps, dispatchProps, ownProps) {
    ...
    return {
        ...
        data: stateProps.data.get(eid) || Immutable.fromJS(ecLocalStorage.getObj("portal-" + window.global_hpid, "blogstatus-" + eid, true) || {}),
        config: Immutable.fromJS(config),
        actions: dispatchProps.actions
    };
}
export default connect(mapStateToProps, mapDispatchToProps, mergeProps)(BlogStatus);
