    ...
    if (obj.img) {
        if (isremind) {
            ...
            if (tempremind.indexOf("0") === -1) {
                return <span className={spanClass}><a href={`javascript:${href};`} onClick={openUrlWin} data-link={obj.link} data-requestid={obj.requestid} title={nametitle}><font className="font"><font style={style}>{obj.pretitle?obj.pretitle:''}{showName}{obj.lasttitle?<b style={{fontWeight:'bold'}}>{obj.lasttitle}</b>:null}</font></font></a></span>
            } else {
                return <span className={spanClass}><a href={`javascript:${href};`} onClick={openUrlWin} data-link={obj.link} data-requestid={obj.requestid} title={nametitle}><font className="font"><font style={style}>{obj.pretitle?obj.pretitle:''}{showName}{obj.lasttitle?<b style={{fontWeight:'bold'}}>{obj.lasttitle}</b>:null}</font></font>&nbsp;</a><img className="wfremindimg" src={obj.img}/></span>
            }
        }
