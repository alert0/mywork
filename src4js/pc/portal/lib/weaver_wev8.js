...
window.checkboxCheck = checkboxCheck;
...
window.checkinpute8 = checkinpute8;
...
window.checkinput4order = checkinput4order;
//判断email格式是否正确
...
window.checkinput_email = checkinput_email;
...
window.checkinput_email_only = checkinput_email_only;
...
window.reshowCheckBox = reshowCheckBox;
/**
...
window.numberChangeToChinese = numberChangeToChinese;
...
window.chineseChangeToNumber = chineseChangeToNumber;
...
window.floatFormat = floatFormat;
...
window.milfloatFormat = milfloatFormat;
...
window.checkinput1 = checkinput1;
...
window.ItemNum_KeyPress = ItemNum_KeyPress;
...
window.ItemPlusCount_KeyPress = ItemPlusCount_KeyPress;
...
window.ItemCount_KeyPress = ItemCount_KeyPress;
...
window.ItemPhone_KeyPress = ItemPhone_KeyPress;
...
function ItemDecimal_KeyPress(elementname, p, s) {
    ...
    if (!(((keyCode >= 48) && (keyCode <= 57)) || keyCode == 46 || keyCode == 45) || (keyCode == 46 && dotCount == 1)) {
        if (evt.keyCode) {
            evt.keyCode = 0;
            stopDefault(evt); //add by liaodong 2013-08-22//evt.returnValue=false;     
        } else {
            evt.which = 0;
            stopDefault(evt); //add by liaodong 2013-08-22//evt.preventDefault();
        }
    }
...
window.ItemDecimal_KeyPress = ItemDecimal_KeyPress;
...
window.getCursortPosition = getCursortPosition;
...
window.checkItemScale = checkItemScale;
...
window.checkcount1 = checkcount1;
...
window.checkcount2 = checkcount2;
...
window.checknumber1 = checknumber1;
...
window.checkPlusnumber1 = checkPlusnumber1;
...
window.checkphone1 = checkphone1;
...
window.ItemTime_KeyPress = ItemTime_KeyPress;
...
window.checkinput_char_num = checkinput_char_num;
...
window.checkdecimal_length = checkdecimal_length;
...
function doSwitch(objTbls) {
    ...
    for (i = 0; i < tblList.length; i++) {
        var tbObj = document.getElementById(tblList[i]);
        if (tbObj == null) continue;
        if (tbObj.tBodies[0].style.display == "none") {
            $(tbObj.tBodies[0]).show();
            tbObj.spanSwitch.innerHTML = "<img src='/images/up_wev8.jpg' style='cursor:pointer'> ";
        } else {
            tbObj.tBodies[0].style.display = "none";
            tbObj.spanSwitch.innerHTML = "<img src='/images/down_wev8.jpg' style='cursor:pointer'>";
        }
    }
}
...
window.change2input = change2input;
...
window.openNewFullWindow = openNewFullWindow;
window.flag = false;
//数据加载未完成的时候 置灰底部tab
...
window.enablebottommenu = enablebottommenu;
...
window.displaybottommenu = displaybottommenu;
...
window.enableAllmenu = enableAllmenu;
...
window.displayAllmenu = displayAllmenu;
...
window.enableAllmenuParent = enableAllmenuParent;
...
function showMsgBox(o, msg, t, l) {
    ...
    o.style.posTop = t || (document.body.offsetHeight / 2 + document.body.scrollTop - 50);
    o.style.posLeft = l || (document.body.offsetWidth / 2 - 50);
}
...
window.realLengthOnly = realLengthOnly;
...
window.checkLengthOnly = checkLengthOnly;
...
window.checkLengthAndCut = checkLengthAndCut;
...
window.checkLengthfortext = checkLengthfortext;
...
window.checkLength4Read = checkLength4Read;
...
window.checkLengthbrow = checkLengthbrow;
...
window.checkLength1 = checkLength1;
...
window.check_number = check_number;
...
window.checkinputnumber = checkinputnumber;
...
function checkinput2(elementname, spanid, viewtype) {
    ...
    //处理金额千分位0--问题
    //取字段类型
    var DBtype = $G(elementname).getAttribute("datatype");

    if (DBtype == "int" || DBtype == "float") {
        if (DBtype == "int") {
        ...
        if (!isNaN(tmpvalue) && tmpvalue != 0) {
            $GetEle(elementname).value = tmpvalue;
        }
    }
    if (viewtype == 1) {
...
window.checkinput2 = checkinput2;
...
function checkinput3(elementname, spanid, viewtype) {
    try {
      ...
      */
        ...
        //处理金额千分位0--问题
        //取字段类型
        var DBtype = elementname.getAttribute("datatype");
...
window.checkinput3 = checkinput3;
...
window.changeshowattr = changeshowattr;
...
window.changeshowattrAjax = changeshowattrAjax;
...
window.uploadAttachmentDisd = uploadAttachmentDisd;
...
window.setBrowserReadOnly = setBrowserReadOnly;
...
window.setFieldReadOnly = setFieldReadOnly;
//联动改变显示属性联动生成的只读对象值
...
window.syncUpdateReadOnlyText = syncUpdateReadOnlyText;
...
window.readyOnlyStyle = readyOnlyStyle;
...
window.getTitleTip = getTitleTip;
...
window.setBrowserOnly = setBrowserOnly;
/**
 *附件不可编辑
 **/
...
window.attachmentDisd = attachmentDisd;
/**
 *附件上传属性联动判断附件是否清除
 **/
...
window.isEmptyfsUpdaload = isEmptyfsUpdaload;
...
window.addSWFUploadRequired = addSWFUploadRequired;
...
window.clearSWFUploadRequired = clearSWFUploadRequired;
...
window.displaySWFUploadError = displaySWFUploadError;
...
window.changeToNormalFormat = changeToNormalFormat;
...
window.changeToThousands2 = changeToThousands2;
...
window.changeToThousands = changeToThousands;
...
window.checkMaxLength = checkMaxLength;
...
window.doshowmrsndiv = doshowmrsndiv;
...
window.enablePhraseselect = enablePhraseselect;
...
window.displayPhraseselect = displayPhraseselect;
...
window.autoFrameSize = autoFrameSize;
/**
打印预览
**/
...
window.printModePreview = printModePreview;
...
window.isMouseLeaveOrEnter = isMouseLeaveOrEnter;
 ...
 * @param extra
 */
function sendMsgToPCorWeb(conventioners, openType, groupName, extra, successCallBack, errorCallBack) {
    jQuery.ajax({
...
window.sendMsgToPCorWeb = sendMsgToPCorWeb;