...
window.FCKEditorExt = CkeditorExt;
...
var UEUtil = {
	...
	initRemark : function (editorid, isIE8,haswfresource) {
		//转发页面特殊处理
		var _ue = UE.getEditor(editorid, {
			 ...
			 autoHeightEnabled : (editorid == 'forwardremark' ? false : true),
	            ...
	            ,'justifyleft', 'justifycenter', 'justifyright', '|',
	            'link', 'unlink', 'insertimage', 'inserttable', '|', '|', 'undo', 'redo','|', haswfresource ? '|' :' ','|'
	        ]], theme : "metro"
       	});
       	this.bindEvent(editorid);
       	_ue.addListener('ready', function(){
			try {
				__insertimage(_ue, "insertimage");
				var fieldannexuploadname = jQuery('#'+editorid+'_div').find("#field-annexupload-name").val();
		        ...
		        */
		         var secId = jQuery('#'+editorid+'_div').find('#annexsecId_param').val();
		         if(jQuery.trim(secId) == "-1"){
		         	alert("请设置签字意见附件上传目录！");
		         }
		       UE.editors.push(editorid);
			} catch (e_4) {console.log(e_4)}
...
//						   paramDiv.find("#fsUploadProgressfileuploaddiv").css({"top":py + "px", "left":px+"px"});
		...
		function createFieldUploader(){
	      ...
	      console.log(">>>>>>>>>>>crmbutton",crmbutton.lenght);
	      if(crmbutton.length===0){
	    	  setTimeout(function(){
	    		  try {
	    			  createUploader();
