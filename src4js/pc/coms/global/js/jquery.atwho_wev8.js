...
Controller = (function() {
  ...
  Controller.prototype.call_default = function() {
    ...
    try {
      ...
      error = _error;
      return $.error("" + error + " Or maybe At_wev8.js doesn't have function " + func_name);
    }
  };
  ...
  Controller.prototype.content = function(with_repeat_matcher) {
    var range;
    if (this.$inputor.is('textarea, input')) {
      ...
      if (!!!with_repeat_matcher) {
	      //选中后不再弹出修改层
	      if (!!range.startContainer.outerHTML) {
	      	return;
	      }
      }
      //window.console.log(range.startContainer.textContent);
  ...
  Controller.prototype.catch_query = function() {
    ...
    content = this.content(this.get_opt('with_repeat_matcher'));
    ...
    //window.console.log(this.get_opt('start_with_space'));
    query = this.callbacks("matcher").call(this, this.at, subtext, this.get_opt('start_with_space'));
    if (typeof query === "string" && query.length <= this.get_opt('max_len', 20)) {
      start = caret_pos - query.length;
      end = start + query.length;
      this.pos = start;
      //window.console.log(query + ", " + start + ", " + end);
      query = {
        'text': query,
        'head_pos': start,
        'end_pos': end
  ...
  Controller.prototype.mark_range = function() {
    ...
    if (this.app.window.getSelection && (sel = this.app.window.getSelection()).rangeCount > 0) {
    	//window.console.log(sel.getRangeAt(0));
  ...
  Controller.prototype.render_view = function(data) {
	...
	var search_key_py = this.get_opt("search_key_py");
    var pydata = [];
    if (search_key_py != '') {
    	pydata = this.callbacks("sorter").call(this, this.query.text, odata.slice(0, 1001), search_key_py);
    	
    	var exids = ',';
    	for (var k=0; k<pydata.length; k++) {
    		exids += pydata[k].id + ',';
    	}
    	
    	for (var j=0;j<data.length; j++) {	
    		if (exids.indexOf(',' + data[j].id + ',') == -1) {
    			pydata.push(data[j]);
    		}
    	}
    	data = pydata;
...
Model = (function() {
  ...
  Model.prototype.query = function(query, callback) {
	...
	var search_key_py = this.context.get_opt("search_key_py");
    var pydata = [];
    if (search_key_py != '') {
    	pydata = this.context.callbacks('filter').call(this.context, query, odata, search_key_py) || [];
    	var exids = ',';
    	for (var k=0; k<pydata.length; k++) {
    		exids += pydata[k].id + ',';
    	}
...
View = (function() {
  ...
  View.prototype.next = function() {
    ...
    return this.$el.animate({
      scrollTop: Math.max(0, cur.outerHeight() * (next.index() + 2) - this.$el.height())
    }, 150);
  };
  ...
  View.prototype.prev = function() {
    ...
    prev.addClass('cur');
    return this.$el.animate({
      scrollTop: Math.max(0, cur.outerHeight() * (prev.index() + 2) - this.$el.height())
    }, 150);
  };
