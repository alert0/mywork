...
import * as EContentAction from '../../actions/formmodecustomsearch/tab';
...
class EContent extends React.Component {
    ...
    render() {
        ...
        if (!_isEmpty(tabdata)) {
             contentHtml = <FmcsTable eid={eid} data={tabdata} esetting={esetting}/>
        }
...
const mapStateToProps = state => {
    const { eformmodecustomsearchtab } = state;
    return ({
        data: eformmodecustomsearchtab.get("data"),
        tabid: eformmodecustomsearchtab.get("tabid"),
        refresh: eformmodecustomsearchtab.get("refresh")
    })
}
...
function mergeProps(stateProps, dispatchProps, ownProps) {
    ...
    return {
        data: stateProps.data.get(key) || Immutable.fromJS(ecLocalStorage.getObj("portal-" + window.global_hpid, "formmodecustomsearch-tab-" + key, true) || initdata),
        tabid: tabid,
