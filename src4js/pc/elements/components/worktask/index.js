...
import * as WorkTaskAction from '../../actions/worktask/';
...
class WorkTask extends React.Component {
    componentDidMount(){
        ...
        actions.initWorkTaskConfig(config.toJSON());
    }
...
WorkTask = WeaTools.tryCatch(React, MyErrorHandler, { error: "" })(WorkTask);
...
function mapDispatchToProps(dispatch) {
    return {
        actions: bindActionCreators(WorkTaskAction, dispatch)
    }
}
...
function mergeProps(stateProps, dispatchProps, ownProps) {
    ...
    return {
        ...
        data: stateProps.data.get(eid) || Immutable.fromJS(ecLocalStorage.getObj("portal-" + window.global_hpid, "worktask-" + eid, true) || {}),
        config: stateProps.config.get(eid) || Immutable.fromJS(ownProps.config),
        actions: dispatchProps.actions
    };
}
export default connect(mapStateToProps, mapDispatchToProps, mergeProps)(WorkTask);
