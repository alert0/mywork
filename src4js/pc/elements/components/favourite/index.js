...
import * as FavouriteAction from '../../actions/favourite/';
...
class Favourite extends React.Component {
    componentDidMount(){
        ...
        actions.initFavouriteConfig(config.toJSON());
    }
...
Favourite = WeaTools.tryCatch(React, MyErrorHandler, { error: "" })(Favourite);
...
function mapDispatchToProps(dispatch) {
    return {
        actions: bindActionCreators(FavouriteAction, dispatch)
    }
}
...
function mergeProps(stateProps, dispatchProps, ownProps) {
    ...
    return {
        ...
        data: stateProps.data.get(eid) || Immutable.fromJS(ecLocalStorage.getObj("portal-" + window.global_hpid, "favourite-" + eid, true) || {}),
        config: stateProps.config.get(eid) || Immutable.fromJS(ownProps.config),
        actions: dispatchProps.actions
    };
}
export default connect(mapStateToProps, mapDispatchToProps, mergeProps)(Favourite);
