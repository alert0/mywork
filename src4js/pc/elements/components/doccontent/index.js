...
import * as DocContentAction from '../../actions/doccontent/';
...
class DocContent extends React.Component {
    componentDidMount(){
        ...
        actions.initDocContentConfig(config.toJSON());
    }
    ...
    render() {
        ...
        if(isHasRight === 'true'){
            if (!_isEmpty(edata)) {
                EContentHtml = <div id={`__content__${eid}`} dangerouslySetInnerHTML={{ __html: edata.data }}></div>
            }
        }else{
...
DocContent = WeaTools.tryCatch(React, MyErrorHandler, { error: "" })(DocContent);
...
function mapDispatchToProps(dispatch) {
    return {
        actions: bindActionCreators(DocContentAction, dispatch)
    }
}
...
function mergeProps(stateProps, dispatchProps, ownProps) {
    ...
    return {
        ...
        data: stateProps.data.get(eid) || Immutable.fromJS(ecLocalStorage.getObj("portal-" + window.global_hpid, "doccontent-" + eid, true) || {}),
        config: stateProps.config.get(eid) || Immutable.fromJS(ownProps.config),
        actions: dispatchProps.actions
    };
}
export default connect(mapStateToProps, mapDispatchToProps, mergeProps)(DocContent);
