...
class NewNotice extends React.Component {
    ...
    render() {
        ...
        if(isHasRight === 'true'){
            if (!_isEmpty(edata)) {
                EContentHtml = <NewNoticeCom eid={eid} data={edata.data} esetting={edata.esetting}/>;
            }
        }else{
...
function mapDispatchToProps(dispatch) {
    return {
        actions: bindActionCreators(NewNoticeAction, dispatch)
    }
}
...
function mergeProps(stateProps, dispatchProps, ownProps) {
    ...
    return {
        ...
        data: stateProps.data.get(eid) || Immutable.fromJS(ecLocalStorage.getObj("portal-" + window.global_hpid, "newnotice-" + eid, true) || {}),
        config: stateProps.config.get(eid) || Immutable.fromJS(ownProps.config),
        actions: dispatchProps.actions
    };
}
export default connect(mapStateToProps, mapDispatchToProps, mergeProps)(NewNotice);
