...
import * as MagazineAction from '../../actions/magazine/';
import Immutable from 'immutable';
import EHeader from '../common/EHeader';
import { NoRightCom } from '../Common';

import MagazineCom from './Magazine';
//元素组件
class Magazine extends React.Component {
    componentDidMount(){
        const { config, actions } = this.props;
        actions.initMagazineConfig(config.toJSON());
    }
    ...
    render() {
        ...
        if(isHasRight === 'true'){
            if (!_isEmpty(edata)) {
                EContentHtml = <MagazineCom eid = {eid} data={edata.data} esetting={edata.esetting}/>
            }
        }else{
...
function mapDispatchToProps(dispatch) {
    return {
        actions: bindActionCreators(MagazineAction, dispatch)
    }
}
...
function mergeProps(stateProps, dispatchProps, ownProps) {
    ...
    return {
        ...
        data: stateProps.data.get(eid) || Immutable.fromJS(ecLocalStorage.getObj("portal-" + window.global_hpid, "magazine-" + eid, true) || {}),
        config: stateProps.config.get(eid) || Immutable.fromJS(ownProps.config),
        actions: dispatchProps.actions
    };
}
export default connect(mapStateToProps, mapDispatchToProps, mergeProps)(Magazine);
