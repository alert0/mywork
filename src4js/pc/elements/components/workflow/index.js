...
import * as WorkflowAction from '../../actions/workflow/';
...
class Workflow extends React.Component {
    componentDidMount(){
        ...
        actions.initWorkflowConfig(config.toJSON());
    }
...
function mapDispatchToProps(dispatch) {
    return {
        actions: bindActionCreators(WorkflowAction, dispatch)
    }
}
...
function mergeProps(stateProps, dispatchProps, ownProps) {
    ...
    return {
        ...
        data: stateProps.data.get(eid) || Immutable.fromJS(ecLocalStorage.getObj("portal-" + window.global_hpid, "workflow-" + eid, true) || {}),
        config: stateProps.config.get(eid) || Immutable.fromJS(ownProps.config),
        actions: dispatchProps.actions
    };
}
export default connect(mapStateToProps, mapDispatchToProps, mergeProps)(Workflow);
