...
import * as MailAction from '../../actions/mail/';
...
class Mail extends React.Component {
    componentDidMount(){
        ...
        actions.initMailConfig(config.toJSON());
    }
...
Mail = WeaTools.tryCatch(React, MyErrorHandler, { error: "" })(Mail);
...
function mergeProps(stateProps, dispatchProps, ownProps) {
    ...
    return {
        ...
        data: stateProps.data.get(eid) || Immutable.fromJS(ecLocalStorage.getObj("portal-" + window.global_hpid, "mail-" + eid, true) || {}),
        config: stateProps.config.get(eid) || Immutable.fromJS(ownProps.config),
        actions: dispatchProps.actions
    };
}
export default connect(mapStateToProps, mapDispatchToProps, mergeProps)(Mail);
