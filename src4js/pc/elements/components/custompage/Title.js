...
import * as CustomPageTitleAction from '../../actions/custompage/title';
...
class TitleContainer extends React.Component {
    ...
    render() {
        ...
        const { tabid, params, config, tabids, titles, handleRefresh, actions } = this.props;
...
const mapStateToProps = state => {
    const { ecustompagetitle } = state;
    return ({
        tabid: ecustompagetitle.get("tabid")
    })
}

function mapDispatchToProps(dispatch) {
    return {
        actions: bindActionCreators(CustomPageTitleAction, dispatch)
    }
}
