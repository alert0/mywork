...
import * as AddWfAction from '../../actions/addwf/';
...
class AddWf extends React.Component {
    componentDidMount(){
        ...
        actions.initAddWfConfig(config.toJSON());
    }
...
AddWf = WeaTools.tryCatch(React, MyErrorHandler, { error: "" })(AddWf);
...
function mapDispatchToProps(dispatch) {
    return {
        actions: bindActionCreators(AddWfAction, dispatch)
    }
}
...
function mergeProps(stateProps, dispatchProps, ownProps) {
    ...
    return {
        ...
        data: stateProps.data.get(eid) || Immutable.fromJS(ecLocalStorage.getObj("portal-" + window.global_hpid, "addwf-" + eid, true) || {}),
        config: stateProps.config.get(eid) || Immutable.fromJS(ownProps.config),
        actions: dispatchProps.actions
    };
}
export default connect(mapStateToProps, mapDispatchToProps, mergeProps)(AddWf);
