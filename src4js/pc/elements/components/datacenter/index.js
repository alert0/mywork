...
class DataCenter extends React.Component {
    ...
    render() {
        ...
        if(isHasRight === 'true'){
            if (!_isEmpty(edata)) {
                EContentHtml = <DataCenterCom data={edata.data} eid={eid} esetting={edata.esetting}/>;
            }
        }else{
...
DataCenter = WeaTools.tryCatch(React, MyErrorHandler, { error: "" })(DataCenter);
...
function mapDispatchToProps(dispatch) {
    return {
        actions: bindActionCreators(DataCenterAction, dispatch)
    }
}
...
function mergeProps(stateProps, dispatchProps, ownProps) {
    ...
    return {
        ...
        data: stateProps.data.get(eid) || Immutable.fromJS(ecLocalStorage.getObj("portal-" + window.global_hpid, "datacenter-" + eid, true) || {}),
        config: stateProps.config.get(eid) || Immutable.fromJS(ownProps.config),
        actions: dispatchProps.actions
    };
}
export default connect(mapStateToProps, mapDispatchToProps, mergeProps)(DataCenter);
