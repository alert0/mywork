...
import * as PictureAction from '../../actions/picture/';
import Immutable from 'immutable';
import EHeader from '../common/EHeader';
import { NoRightCom } from '../Common';
import PictureCom from './Picture';
//元素组件
class Picture extends React.Component {
    componentDidMount(){
        const { config, actions } = this.props;
        actions.initPictureConfig(config.toJSON());
    }
    ...
    render() {
        ...
        if(isHasRight === 'true'){
            if (!_isEmpty(edata)) {
                EContentHtml = <PictureCom eid={eid} data={edata.data} esetting={edata.esetting}/>
            }
        }else{
...
Picture = WeaTools.tryCatch(React, MyErrorHandler, { error: "" })(Picture);
...
function mapDispatchToProps(dispatch) {
    return {
        actions: bindActionCreators(PictureAction, dispatch)
    }
}
...
function mergeProps(stateProps, dispatchProps, ownProps) {
    ...
    return {
        ...
        data: stateProps.data.get(eid) || Immutable.fromJS(ecLocalStorage.getObj("portal-" + window.global_hpid, "picture-" + eid, true) || {}),
        config: stateProps.config.get(eid) || Immutable.fromJS(ownProps.config),
        actions: dispatchProps.actions
    };
}
export default connect(mapStateToProps, mapDispatchToProps, mergeProps)(Picture);
