...
import * as RssAction from '../../actions/rss/';
...
class Rss extends React.Component {
    componentDidMount(){
        ...
        actions.initRssConfig(config.toJSON());
    }
...
function mapDispatchToProps(dispatch) {
    return {
        actions: bindActionCreators(RssAction, dispatch)
    }
}
...
function mergeProps(stateProps, dispatchProps, ownProps) {
    ...
    return {
        ...
        data: stateProps.data.get(eid) || Immutable.fromJS(ecLocalStorage.getObj("portal-" + window.global_hpid, "rss-" + eid, true) || {}),
        config: stateProps.config.get(eid) || Immutable.fromJS(ownProps.config),
        actions: dispatchProps.actions
    };
}
export default connect(mapStateToProps, mapDispatchToProps, mergeProps)(Rss);
