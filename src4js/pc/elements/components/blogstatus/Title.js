...
import * as BlogStatusTitleAction from '../../actions/blogstatus/title';
...
class TitleContainer extends React.Component {
    ...
    render() {
        ...
        const { tabid, config, tabids, titles, counts, actions, handleRefresh } = this.props;
...
const mapStateToProps = state => {
    const { eblogstatustitle } = state;
    return ({
        tabid: eblogstatustitle.get("tabid")
    })
}

function mapDispatchToProps(dispatch) {
    return {
        actions: bindActionCreators(BlogStatusTitleAction, dispatch)
    }
}
