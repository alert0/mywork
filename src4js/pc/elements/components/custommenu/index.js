...
import * as CustomMenuAction from '../../actions/custommenu/';
import Immutable from 'immutable';
import EHeader from '../common/EHeader';
import { NoRightCom } from '../Common';
import CustomMenuCom from './CustomMenu';
//元素组件
class CustomMenu extends React.Component {
    componentDidMount(){
        const { config, actions } = this.props;
        actions.initCustomMenuConfig(config.toJSON());
    }
    ...
    render() {
        ...
        if(isHasRight === 'true'){
            if (!_isEmpty(edata)) {
                EContentHtml = <CustomMenuCom data={edata.data} eid={eid} esetting={edata.esetting}/>;
            }
        }else{
...
function mapDispatchToProps(dispatch) {
    return {
        actions: bindActionCreators(CustomMenuAction, dispatch)
    }
}
...
function mergeProps(stateProps, dispatchProps, ownProps) {
    ...
    return {
        ...
        data: stateProps.data.get(eid) || Immutable.fromJS(ecLocalStorage.getObj("portal-" + window.global_hpid, "custommenu-" + eid, true) || {}),
        config: stateProps.config.get(eid) || Immutable.fromJS(ownProps.config),
        actions: dispatchProps.actions
    };
}
export default connect(mapStateToProps, mapDispatchToProps, mergeProps)(CustomMenu);
