...
import * as OutDataAction from '../../actions/outdata/';
...
class OutData extends React.Component {
    componentDidMount(){
        ...
        actions.initOutDataConfig(config.toJSON());
    }
...
OutData = WeaTools.tryCatch(React, MyErrorHandler, { error: "" })(OutData);
...
function mapDispatchToProps(dispatch) {
    return {
        actions: bindActionCreators(OutDataAction, dispatch)
    }
}
...
function mergeProps(stateProps, dispatchProps, ownProps) {
    ...
    return {
        ...
        data: stateProps.data.get(eid) || Immutable.fromJS(ecLocalStorage.getObj("portal-" + window.global_hpid, "outdata-" + eid, true) || {}),
        config: stateProps.config.get(eid) || Immutable.fromJS(ownProps.config),
        actions: dispatchProps.actions
    };
}
export default connect(mapStateToProps, mapDispatchToProps, mergeProps)(OutData);
