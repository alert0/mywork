...
import { reqPictureDatas } from '../../apis/picture';
import Immutable from 'immutable';
import ecLocalStorage from '../../util/ecLocalStorage';
const initPictureConfig = (config) => {
    return (dispatch, getState) => {
        ...
        dispatch(getPictureDatas(params));
    }
}
const getPictureDatas = (params) => {
    return (dispatch, getState) => {
        const { eid } = params;
        reqPictureDatas(params).then((data) => {
            let iolddata = getState().elements.get("data");
            let idata = Immutable.fromJS(data);
            ecLocalStorage.set("portal-" + window.global_hpid, "picture-" + eid, data, true);
