...
import { reqAudioDatas } from '../../apis/audio';
import Immutable from 'immutable';
import ecLocalStorage from '../../util/ecLocalStorage';
const initAudioConfig = (config) => {
    return (dispatch, getState) => {
        ...
        dispatch(getAudioDatas(params));
    }
}
const getAudioDatas = (params) => {
    return (dispatch, getState) => {
        const { eid } = params;
        reqAudioDatas(params).then((data) => {
            let iolddata = getState().elements.get("data");
            let idata = Immutable.fromJS(data);
            ecLocalStorage.set("portal-" + window.global_hpid, "audio-" + eid, data, true);
