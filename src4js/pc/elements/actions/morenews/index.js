...
import { reqMoreNewsDatas } from '../../apis/morenews';
import Immutable from 'immutable';
import ecLocalStorage from '../../util/ecLocalStorage';
const initMoreNewsConfig = (config) => {
    return (dispatch, getState) => {
        ...
        dispatch(getMoreNewsDatas(params));
    }
}
const getMoreNewsDatas = (params) => {
    return (dispatch, getState) => {
        const { eid } = params;
        reqMoreNewsDatas(params).then((data) => {
            let iolddata = getState().elements.get("data");
            let idata = Immutable.fromJS(data);
            ecLocalStorage.set("portal-" + window.global_hpid, "morenews-" + eid, data, true);
