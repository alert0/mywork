...
import { reqWorkTaskDatas } from '../../apis/worktask';
import Immutable from 'immutable';
import ecLocalStorage from '../../util/ecLocalStorage';
const initWorkTaskConfig = (config) => {
    return (dispatch, getState) => {
        ...
        dispatch(getWorkTaskDatas(params));
    }
}
const getWorkTaskDatas = (params) => {
    return (dispatch, getState) => {
        const { eid } = params;
        reqWorkTaskDatas(params).then((data) => {
            let iolddata = getState().elements.get("data");
            let idata = Immutable.fromJS(data);
            ecLocalStorage.set("portal-" + window.global_hpid, "worktask-" + eid, data, true);
