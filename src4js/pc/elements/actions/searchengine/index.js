...
import { reqSearchEngineDatas } from '../../apis/searchengine';
import Immutable from 'immutable';
import ecLocalStorage from '../../util/ecLocalStorage';
const initSearchEngineConfig = (config) => {
    return (dispatch, getState) => {
        ...
        dispatch(getSearchEngineDatas(params));
    }
}
const getSearchEngineDatas = (params) => {
    return (dispatch, getState) => {
        const { eid } = params;
        reqSearchEngineDatas(params).then((data) => {
            let iolddata = getState().elements.get("data");
            let idata = Immutable.fromJS(data);
            ecLocalStorage.set("portal-" + window.global_hpid, "searchengine-" + eid, data, true);
