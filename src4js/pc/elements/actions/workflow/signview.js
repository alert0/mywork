...
const signviewPoistion = (eid) => {
	return (dispatch, getState) => {
		...
		jQuery("#tabcontant_" + eid).find(".imgdetail").bind("click",function() {
		    ...
		    signPreview.find(".signTitle").html("<a style='color:#000' title='"+title+"' href='javascript:openWorkflowWin(\""+eid+"\",\""+link+"\",\"2\")'>"+title+"</a>");
		    let pagesize = 3;
		    let curPage = 1;
		    let totalCount = 0;
	        var params = {
		    ...
		    const pStr = "?requestid="+requestid+"&actiontype=requestLog&maxrequestlogid="+maxrequestlogid+"&loadmethod=portal&firstload=true&requestLogParams=&wfsignlddtcnt="+pagesize+"&pgnumber="+curPage;
...
const getMoreSignData = (eid) =>{
	...
	return (dispatch, getState) => {
		...
		var pagesize = 3;
		var pStr = "?requestid="+requestid+
		"&actiontype=requestLog&maxrequestlogid="+maxrequestlogid+
