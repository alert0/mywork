...
import { reqJobsInfoDatas } from '../../apis/jobsinfo';
import Immutable from 'immutable';
import ecLocalStorage from '../../util/ecLocalStorage';
const initJobsInfoConfig = (config) => {
    return (dispatch, getState) => {
        ...
        dispatch(getJobsInfoDatas(params));
    }
}
const getJobsInfoDatas = (params) => {
    return (dispatch, getState) => {
        const { eid } = params;
        reqJobsInfoDatas(params).then((data) => {
            let iolddata = getState().elements.get("data");
            let idata = Immutable.fromJS(data);
            ecLocalStorage.set("portal-" + window.global_hpid, "jobsinfo-" + eid, data, true);
