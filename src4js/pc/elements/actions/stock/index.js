...
import { reqStockDatas } from '../../apis/stock';
import Immutable from 'immutable';
import ecLocalStorage from '../../util/ecLocalStorage';
const initStockConfig = (config) => {
    return (dispatch, getState) => {
        ...
        dispatch(getStockDatas(params));
    }
}
const getStockDatas = (params) => {
    return (dispatch, getState) => {
        const { eid } = params;
        reqStockDatas(params).then((data) => {
            let iolddata = getState().elements.get("data");
            let idata = Immutable.fromJS(data);
            ecLocalStorage.set("portal-" + window.global_hpid, "stock-" + eid, data, true);
