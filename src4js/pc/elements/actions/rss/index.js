...
import { reqRssDatas } from '../../apis/rss';
import Immutable from 'immutable';
import ecLocalStorage from '../../util/ecLocalStorage';

import { initRssTabDatas } from './tab';
import { initTitleData } from './title';
const initRssConfig = (config) => {
    return (dispatch, getState) => {
        ...
        dispatch(getRssDatas(params));
    }
}
const getRssDatas = (params) => {
    return (dispatch, getState) => {
        const { eid } = params;
        reqRssDatas(params).then((data) => {
            let iolddata = getState().elements.get("data");
            let idata = Immutable.fromJS(data);
            ecLocalStorage.set("portal-" + window.global_hpid, "rss-" + eid, data, true);
            dispatch(handleImmutableData(eid, false, data));
            dispatch(initTitleData(eid,data.currenttab));
            dispatch(initRssTabDatas(eid, data.currenttab, data.data));
        });
    }
}
