...
class Forward extends React.Component {
	constructor(props) {
		...
		this.state = {
			...
			loading: false,
			field5: '',
			selectOperatorsMax: 100 //转发人数最大限制，默认100
		};
	}
	...
	componentWillReceiveProps(nextProps) {
		...
		if(showForward && !this.props.showForward) {
			...
			if(forwardOperators != '') {
				this.setOperatorIds(forwardOperators, false);
			}
		}
	}
	...
	//更新操作者
	setOperatorIds(ids, isAllUser, count) {
		if(isAllUser) {
			const allUser = [{ type: 'all', lastname: '所有人', ids: ids, count: count, id: 'all_users' }];
