...
class FormLayoutTr extends React.Component {
    ...
    render() {
        const {actions,symbol,colNum,rowid,rowHeight,rowCusAttr,colattrs,ecMap,layout,conf,mainData,detailData,fieldVariable} = this.props;
        const {addColType,detailRowValue} = this.props;      //明细解析相关
        let colid = 0;
            ...
            if(findRowSpan(rowid,colid,ecMap)) {  //行合并单元格不生成DOM
                ...
                if(colid === 0 && symbol.indexOf("detail_") > -1 && addColType > 0){  //低级模式增加序号列
                    if(addColType === 1)
                        ...
                        cellArr.push(<td style={tdCellAttr.styleObj}><div style={tdCellAttr.innerStyleObj}>
                            <Checkbox disabled /><span>{detailRowValue.get("serialnum")}</span></div></td>);
                }
