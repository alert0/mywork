...
export const doSearch = (params = {}) => {
	return(dispatch, getState) => {
		...
		const paramsBase = getState()['workflow' + viewScope].get('paramsBase').toJS();
		const searchParams = getState()['workflow' + viewScope].get('searchParams').toJS();
		const searchParamsAd = getState()['workflow' + viewScope].get('searchParamsAd').toJS();
		const leftTreeCount = getState()['workflow' + viewScope].get('leftTreeCount').toJS();
...
export const batchSubmitClick = params => {
	return(dispatch, getState) => {
		const inputSign = getState().workflowlistDoing.getIn(["sharearg","multisubmitnotinputsign"]);
		if(inputSign == "0") //弹窗输入签字意见
...
export const batchSubmitWf = (remark, checkedKeys) => {
	return(dispatch, getState) => {
		const viewScope = getState().workflowlistDoing.get('nowRouterWfpath');
		dispatch({ type: viewScope + '_' + types.LOADING, loading: true });
		const comsWeaTable = getState().comsWeaTable;
		const datas = comsWeaTable.getIn([comsWeaTable.get('tableNow'), 'datas']).toJS();
