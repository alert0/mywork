...
class ImgSlide extends React.Component {
	...
	render() {
		const { eid, data } = this.props;
		const {imgsrclist,imgdesclist,height,rowcount,displaydesc,cellwidth} = data;
		...
		imgdesclist.map((item,i)=>{
			...
			return <div id={`imgslide_${eid}`} style={{width: '100%',height:height}}>
				  <div id={`banner_${eid}`} className="slidebox_block" style={{height: '100%'}}>
				    <div style={{position: 'absolute', height: '28px', width: '28px', zIndex: 999, top: '10px', right: '10px', cursor: 'pointer', display: 'none'}}>
				      <img src="/page/element/imgSlide/resource/image/ssetting.png" height="28px" width="28px" id={`settingico_${eid}`} title="设置" /></div>
