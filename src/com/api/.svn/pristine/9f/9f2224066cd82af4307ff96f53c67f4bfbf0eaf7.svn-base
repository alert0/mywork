...
public class LayoutInfoService {
	 ...
	 * 获取解码后代码块内容
	 */
	public String getLayoutScripts(int layoutid){
		String scriptContent = "";
		if(layoutid > 0){
			RecordSet rs = new RecordSet();
			rs.executeSql("select scripts,scriptsE9 from workflow_nodehtmllayout where id="+layoutid);
			if(rs.next()){
				String str = Util.null2String(rs.getString("scripts"));
				ParseExcelLayout parse = new ParseExcelLayout(new HashMap<String,String>());
				if(!"".equals(str))
					scriptContent = parse.decodeStr(str);
				str = Util.null2String(rs.getString("scriptsE9"));
				if(!"".equals(str))
					scriptContent += parse.decodeStr(str);
			}
		}
				...
				if(etype == 5){
					...
					if(nodeidList.indexOf(remarknodeid+"") > -1){
						if (noderemarkmap.containsKey("node"+remarknodeid)) {
							...
							nodemark = ServiceUtil.manageImgLazyLoad(nodemark);
							// 去掉多余分隔符
