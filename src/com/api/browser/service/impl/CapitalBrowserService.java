	...
	public Map<String, Object> getBrowserData(Map<String, Object> params) throws Exception {
		...
		if (!capitalgroupid.equals("") && !capitalgroupid.equals("0")) {
			if ("sqlserver".equalsIgnoreCase(rs.getDBType())) {
				...
				mysql += "SELECT id," + "        supassortmentid" + " FROM   cptcapitalassortment" + " WHERE  id = " + capitalgroupid + " UNION ALL" + " SELECT a.id," + "        a.supassortmentid"
						+ " FROM   cptcapitalassortment AS a," + "        cptgroupinfo AS b" + " WHERE  a.supassortmentid = b.id";
				...
				sqlwhere += " and exists(select 1 from (select ts1.id from CPTCAPITALASSORTMENT ts1 start with ts1.ID = '" + capitalgroupid
						+ "' connect by prior ts1.id = ts1.SUPASSORTMENTID) ts2 where ts2.id=T1.CAPITALGROUPID ) ";
			} else {
		...
		if (!"".equals(queryformjson)) {
			...
			while (rs_condition.next()) {
				...
				if (needHideField.contains("," + fieldname + ","))
					continue;
				...
				if (currentnodetype > 0 && currentnodetype < 3) {
					...
					if (jsonObject.length() > 0) {
						...
						if (!cptdetailtablename.equals(cptmaintablename)) {
							...
							cptsearchsql = "select m." + zcname + " as currentzcid,sum(m." + zcsl + ") as currentreqnum from " + cptmaintablename + " m  where  m.requestid=" + reqid + " group by m."
									+ zcname + " ";
						}

						backfields = " t2.currentreqnum,t1.*,"
								+ ("oracle".equalsIgnoreCase(rs.getDBType()) ? "(nvl(capitalnum,0)-nvl(frozennum,0)+nvl(currentreqnum,0))"
										: "(isnull(capitalnum,0)-isnull(frozennum,0)+isnull(currentreqnum,0))") + " cptnum ";
		...
		// out.println("sql:\n select "+backfields+" from "+fromSql+"
		// "+sqlwhere+includeNumZeroSqlwhere+" order by "+orderby);
		...
		sqlwhere = sqlwhere + includeNumZeroSqlwhere;

		List<SplitTableColBean> cols = new ArrayList<SplitTableColBean>();
		SplitTableColBean colBean = new SplitTableColBean("true", "id");
		colBean.setTransmethod("weaver.cpt.util.CapitalTransUtil.getBrowserRetInfo");
		colBean.setOtherpara(isCapital);
		cols.add(colBean);
		...
		while (rs.next()) {
			...
			String otherpara = user.getUID() + "+" + fieldid + "+" + fieldhtmltype + "+" + type;
			cols.add(new SplitTableColBean("5%", SystemEnv.getHtmlLabelName(fieldlabel, user.getLanguage()), fieldname, fieldname, "weaver.cpt.util.CptFieldManager.getBrowserFieldvalue", otherpara));
		}
